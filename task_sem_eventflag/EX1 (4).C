...
char get_letter[4];//W or L을 저장 decision task에서 만든 문자를 Random task1~4중 하나가 받는다.
	...
	if (*(char*)pdata == '4') {//decision task일 경우
		for (;;) {
			...
			for (i = 0; i < N_TASK - 1; i++) {
				...
				get_letter[i] = push_letter;
				

				
			}
			OSSemPost(sem);
			OSFlagPost(g_grp, 0x0F, OS_FLAG_SET, &err);
		...
		for (;;) {
			...
			get_number[task_number] = push_number;
				
				
			OSFlagPost(e_grp, 1 << task_number, OS_FLAG_SET, &err);
			OSFlagPend(g_grp, 1 << task_number, OS_FLAG_WAIT_SET_ALL + OS_FLAG_CONSUME, 0, &err);
			OSSemPost(sem);
			...
			PC_DispChar(9 + 18 * task_number, 4, get_letter[task_number], DISP_FGND_BLACK + DISP_BGND_LIGHT_GRAY);

			
			if (get_letter[task_number] == 'W') {
				for (j = 5; j < 24; j++) {
					for (i = 0; i < 80; i++) {
